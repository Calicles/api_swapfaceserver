...
FaceSwapper::FaceSwapper(const std::vector<unsigned char> &img1, const std::string &fileNameImg2) 
    : m_img1(1, img1.size(), CV_8UC3, (void*) img1.data()),
...
void FaceSwapper::applyAffineTransform(Mat &warpImage, Mat &src, std::vector<Point2f> &srcTri, std::vector<Point2f> &dstTri) {
    Mat warpMat = getAffineTransform( srcTri, dstTri );

    warpAffine( src, warpImage, warpMat, warpImage.size(), INTER_LINEAR, BORDER_REFLECT_101);
}

void FaceSwapper::calculateDelaunayTriangles(Rect rect, std::vector<cv::Point2f> &points, std::vector< std::vector<int> > &delaunayTri) {
